...
Pane 離開 `awaiting` 或 `question` 就算作它的答案已被採納，而未送出的那一行不論
...
停泊的請求帶著一個在 Backend 啟動時鑄造的 Token。它是新鮮度檢查，不是授權邊界 ——
它就放在 hook 所在的同一個設定檔裡，任何以你的身分執行的東西都讀得到 —— 而它換來
的是：來自更早一次 Backend 執行的殘留 hook，無法停泊在現在屬於另一個 Session 的
Pane 上。
...
要重新啟動。Claude 的通道是它自己設定檔裡的一個 hook：把它關掉會立即停止 Navide
使用它，而 hook 本身會在 Backend 下次重新啟動時被移除。
