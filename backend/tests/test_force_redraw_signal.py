...
@pytest.fixture
def terminal_event_loop():
    """Give synchronous TerminalService tests an explicitly owned loop."""
...
def test_force_redraw_signals_even_at_an_unchanged_size(terminal_event_loop) -> None:
...
@pytest.mark.skipif(sys.platform == "win32", reason="pty/SIGWINCH are POSIX-only")
def test_force_redraw_leaves_the_requested_size_in_place(terminal_event_loop) -> None:
